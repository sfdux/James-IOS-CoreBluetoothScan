...
- (void)setupCoreBluetooth{
    [BlueToothHelper shared].discoveredPeripherals = [NSMutableArray new];
    [BlueToothHelper shared].delegate = self;
    [[BlueToothHelper shared] startScan];
}
...
//    CGRect btnFrame = CGRectMake(0,self.bounds.origin.y - 32.0f,xWidth,self.bounds.size.height - tableFrame.size.height - 32.0f);
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[BlueToothHelper shared] connectPeripheral:[BlueToothHelper shared].discoveredPeripherals[indexPath.row]];
}
...
#pragma mark - Blue tooth helper delegate
- (void)blueToothHelper: (CBCentralManagerState) state
        didNotSupported:(NSError *)error{
    NSLog(@"didNotSupported: error = %@", [error localizedDescription]);
}

-(void) blueToothHelper:(NSMutableArray *)peripherals didDiscovered:(NSError *)error{
    [self.mainPopoverListView reloadData];
}

- (void)blueToothHelper:  (CBPeripheral *)peripheral
           didConnected:(NSError *)error{
    NSLog(@"didConnected: peripheral = %@", peripheral);
    //Only for WAX9 sensor
    [[BlueToothHelper shared] sendCommand:peripheral command:1];
}

- (void)blueToothHelper:  (CBPeripheral *)peripheral
     didFailToConnected:(NSError *)error{
    NSLog(@"didFailToConnected: peripheral = %@ and error is %@", peripheral,error);
}

- (void)blueToothHelper:  (CBPeripheral *)peripheral
          didWriteValue:(NSError *)error{
    [[BlueToothHelper shared] subscriptSensorData:peripheral];
}
...
- (void)blueToothHelper:  (CBPeripheral *)peripheral
    ...
    const char* fileBytes =[value bytes];
    ...
    {
        aByte = fileBytes[index];
        data = [data stringByAppendingString: [NSString stringWithFormat:@"%ld: \t %p \n", (long)index, aByte]];
        NSLog(@"%ld: %p", (long)index, aByte);
        [dataArray addObject:[NSString stringWithFormat:@"%p", aByte]];
    }
    
    [self writeToFile:value FileName:@"/test"];
